...
function SendTab({ user }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      xhr.onerror = () => { setUploading(false); setError("Network error"); showNotification("Network error", "error"); };
      xhr.send(formData);
    } catch (err) {
      setUploading(false); setError(err.message); showNotification(err.message, "error");
    }
  };

  const copyPasscode = async () => {
    await navigator.clipboard.writeText(result.passcode);
    setCopied(true); showNotification("Passcode copied!", "success");
  ...
  return (
    <div>
      ...
      {isGuest && (
        <div className="mb-5 p-4 rounded-xl border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10 fade-in">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Link to="/login" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-white text-xs font-semibold transition-all duration-200 shrink-0">
        ...
        <div className={`mb-5 p-4 rounded-xl border transition-colors duration-300 ${
          ...
          <div className="flex items-center justify-between mb-2">
            <span className="flex items-center gap-1.5 text-sm font-semibold text-slate-800 dark:text-slate-300">
              <ChartBarIcon className="w-4 h-4 text-indigo-500" /> Monthly Storage
            </span>
          ...
          <div className="w-full h-2 bg-slate-100 dark:bg-white/8 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${
                isAtLimit ? "bg-linear-to-r from-rose-500 to-orange-500"
                  : usedPct > 75 ? "bg-linear-to-r from-amber-500 to-orange-400"
          ...
          {isAtLimit && (
            <p className="text-xs text-rose-500 dark:text-rose-400 mt-1.5">
              Storage full.{" "}
              <button onClick={() => setShowUpgrade(true)} className="underline font-medium hover:text-rose-600">
                Upgrade to continue
              </button>
            </p>
          )}
      ...
      <div className="bg-white dark:bg-white/3 border border-slate-300 dark:border-white/8 rounded-2xl p-6 shadow-md dark:shadow-2xl transition-colors duration-300">
        <DragDropUpload onFileSelect={setFile} disabled={uploading || isAtLimit} maxBytes={isGuest ? GUEST_LIMIT : FREE_LIMIT} />
      ...
      {result && (
        <div className="mt-6 bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-200 dark:border-emerald-500/20 rounded-2xl p-6 fade-in">
          ...
          <p className="text-slate-700 dark:text-slate-400 text-sm mb-1 truncate">{result.filename}</p>
          ...
          {isGuest && (
            <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 rounded-xl text-center">
              <p className="text-amber-700 dark:text-amber-400 text-xs">
                ...
                <Link to="/login" className="font-semibold underline hover:text-amber-600">
...
function ReceiveTab() {
  ...
  const handleKeyDown = (index, e) => {
    if (e.key === "Backspace" && !passcode[index] && index > 0) inputRefs.current[index - 1]?.focus();
    if (e.key === "Enter") handleFetch();
  };

  const handlePaste = (e) => {
    const pasted = e.clipboardData.getData("text").replace(/\D/g, "").slice(0, 6);
    if (pasted.length === 6) { setPasscode(pasted.split("")); inputRefs.current[5]?.focus(); }
    e.preventDefault();
  };
  ...
  const handleFetch = async () => {
    ...
    setError(""); setLoading(true); setFileInfo(null);
    try {
  ...
  return (
    <div>
      ...
      <div className="bg-white dark:bg-white/3 border border-slate-300 dark:border-white/8 rounded-2xl p-5 sm:p-6 shadow-md dark:shadow-2xl mb-5 transition-colors duration-300">
        <p className="text-slate-700 dark:text-slate-400 text-sm text-center mb-4">Enter 6-digit passcode</p>
          ...
          <div className="bg-white dark:bg-white/3 border border-slate-300 dark:border-white/10 rounded-2xl p-5 shadow-sm transition-colors duration-300">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-600 dark:text-slate-400 text-sm">{formatSize(fileInfo.size)}</p>
      ...
      <div className="max-w-2xl mx-auto px-4 py-10 sm:py-12 pb-24 sm:pb-12">
        ...
        <div className="text-center mb-8 fade-in">
          ...
          <p className="text-slate-700 dark:text-slate-400 text-sm sm:text-base">
            {user ? (
              <>Upload files up to <span className="font-medium text-indigo-500">8 GB</span>. Files are <span className="font-medium text-rose-500 dark:text-rose-400">permanently deleted</span> after the first download.</>
            ) : (
              <>Upload files up to <span className="font-medium text-amber-500">500 MB</span> as a guest. <span className="font-medium text-rose-500 dark:text-rose-400">Auto-deleted</span> after download.</>
            )}
          </p>
        </div>
